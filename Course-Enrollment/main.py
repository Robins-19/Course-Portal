...
def signup1():
    ...
    connection = connect(host="localhost", database="test1", user="root", password="Robins", port='3305')
...
def c9():
    ...
    connection = connect(host="localhost", database="test1", user="root", password="Robins", port='3305')
    cur = connection.cursor()
    query = "Update courses set score='{}' where email='{}' and courseid='{}'".format(score, email, '1111')
    cur.execute(query)
    connection.commit()
    data=score
...
def cpp10():
    ...
    connection = connect(host="localhost", database="test1", user="root", password="Robins", port='3305')
    cur = connection.cursor()
    query = "Update courses set score='{}' where email='{}' and courseid='{}'".format(score, email,'5555')
    cur.execute(query)
    connection.commit()
    return str(score * 10)
...
def python10():
    ...
    connection = connect(host="localhost", database="test1", user="root", password="Robins", port='3305')
    cur = connection.cursor()
    query = "Update courses set score='{}' where email='{}' and courseid='{}'".format(score, email,'3333')
    cur.execute(query)
    connection.commit()
    return str(score * 10)
...
def Java1():
    return render_template('java-introduction.html')

@app.route('/Java-Variable')
def Java2():
    return render_template('java-variable.html')

@app.route('/Java-loop')
def Java3():
    return render_template('java-loop.html')

@app.route('/Java-object-class')
def Java4():
    return render_template('Java-object-class.html')

@app.route('/Java-Quiz')
def Java5():
    return render_template('javaQuiz.html')

@app.route('/Java-Quiz',methods=['post'])
def java6():
    score = 0
    ans = []
    for i in range(1,11):
        ans.append(request.form.get('Q'+str(i)))
    ansKey = ['C','B','C','B','D','B','C','B','A','C']
    for i in range(0,10):
        if(ans[i] == ansKey[i]):
            score= score+1
    print(score)
    ...
    connection = connect(host="localhost", database="test1", user="root", password="Robins", port='3305')
    cur = connection.cursor()
    query = "Update courses set score='{}' where email='{}' and courseid='{}'".format(score, email,'2222')
    cur.execute(query)
    connection.commit()
    return str(score*10)
